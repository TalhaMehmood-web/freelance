...
export async function GET(req: NextRequest) {
  ...
  const rows = await db.permissionAction.findMany({ orderBy: { slug: "asc" } })
...
export async function POST(req: NextRequest) {
  ...
  if (existing) return NextResponse.json({ error: `Action "${slug}" already exists.` }, { status: 400 })
...
export async function DELETE(req: NextRequest) {
  ...
  const { count } = await db.permissionAction.deleteMany({})
