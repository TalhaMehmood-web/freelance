...
export function ActionsTable({ isSuperAdmin }: Props) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<PermissionActionRow | null>(null)
  ...
  const deleteAllMutation = useMutation({
    mutationFn: () => axios.delete("/api/admin/actions"),
    onSuccess: (res) => {
      toast.success(`All ${res.data.deleted} actions deleted.`)
      setDeleteAllOpen(false)
      queryClient.invalidateQueries({ queryKey: ["admin-actions"] })
    },
    onError: (err: any) => {
      toast.error(err?.response?.data?.error ?? "Failed to delete all actions.")
    },
  })
  ...
  return (
    <div className="bg-surface rounded-2xl border border-border shadow-card flex flex-col gap-0 overflow-hidden">
      ...
      <DeleteConfirmDialog
        open={!!deleteTarget}
        title="Delete Action"
        description={deleteTarget ? `Delete action "${deleteTarget.slug}"? Existing permissions using it will not be affected.` : ""}
      ...
      <DeleteConfirmDialog
        ...
        description="This will permanently delete every action. Existing permissions using them will not be affected."
