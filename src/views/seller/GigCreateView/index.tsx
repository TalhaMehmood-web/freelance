...
import { Step4Gallery } from "./Step4Gallery"
...
export function GigCreateView({ categories = [] }: GigCreateViewProps) {
  ...
  const [descriptionData, setDescriptionData] = useState<GigDescriptionData | null>(null)
  const [, setGalleryData]                    = useState<GigGalleryData | null>(null)

  const basicsForm = useForm<GigBasicsData>({
    resolver: zodResolver(GigBasicsSchema),
    defaultValues: basicsData ?? { title: "", categoryId: "", subcategoryId: "", searchTags: [] },
  })

  const pricingForm = useForm<GigPricingData>({
    resolver: zodResolver(GigPricingSchema),
    defaultValues: pricingData ?? {
      basic:    { enabled: true,  name: "", description: "", deliveryDays: 7,  revisions: 2,  priceCents: 0 },
      standard: { enabled: false, name: "", description: "", deliveryDays: 14, revisions: 5,  priceCents: 0 },
      premium:  { enabled: false, name: "", description: "", deliveryDays: 30, revisions: 99, priceCents: 0 },
    },
  })

  const descriptionForm = useForm<GigDescriptionData>({
    resolver: zodResolver(GigDescriptionSchema),
    defaultValues: descriptionData ?? { description: "", faqs: [] },
  })
  ...
  async function handleNext() {
    if (currentStep === 1) {
      ...
      setGalleryData(galleryForm.getValues())
    }
    ...
    <div className="flex flex-col gap-0 min-h-full">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-text-primary leading-none">Create a New Gig</h1>
