  ...
  return (
    <div className="flex items-start gap-4 py-4 border-b border-border last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs font-semibold text-text-tertiary uppercase tracking-wide mb-0.5">
          {label}
        </p>
      ...
      {complete ? (
        ...
        <span className="text-xs text-warning-500 font-medium shrink-0 mt-1">
          Incomplete
        </span>
      )}
    </div>
...
export function Step5Publish({
  ...
  const enabledPackages = pricingData
    ? (["basic", "standard", "premium"] as const).filter(
        (k) => pricingData[k].enabled,
      )
    : [];
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-text-secondary">
          Review your gig details before publishing. You can always edit after
          publishing.
        </p>
      </div>

      <div className="bg-surface rounded-2xl border border-border shadow-car px-4">
        <ReviewRow
          icon={<Tag className="h-4 w-4 text-success-500" />}
          label="Overview"
          complete={
            !!basicsData?.title &&
            !!basicsData?.categoryId &&
            (basicsData?.searchTags?.length ?? 0) > 0
          }
          value={
            basicsData ? (
              <div>
                ...
                <p className="text-xs text-text-tertiary mt-0.5">
                  ...
                  {basicsData.searchTags?.length > 0 &&
                    ` · ${basicsData.searchTags.join(", ")}`}
                </p>
              </div>
            ) : (
              "Not completed"
              ...
              <div className="flex gap-3 flex-wrap">
                {enabledPackages.map((k) => {
                  ...
                  return (
                    <span
                      key={k}
                      className="text-xs bg-surface-subtle border border-border rounded-lg px-2.5 py-1"
                    >
                      <span className="font-medium capitalize">{k}</span> —{" "}
                      {formatCurrency(pkg.priceCents)} · {pkg.deliveryDays}d
                    </span>
                  );
                })}
              </div>
      ...
      <div className="bg-brand-50 border border-brand-100 rounded-xl p-4">
        <p className="text-sm text-brand-700 leading-relaxed">
          <strong>Ready to publish?</strong> Once published, your gig will be
          visible in the marketplace. You&apos;ll receive an email confirmation
          and your seller metrics will start tracking immediately.
        </p>
      </div>
    </div>
  );
